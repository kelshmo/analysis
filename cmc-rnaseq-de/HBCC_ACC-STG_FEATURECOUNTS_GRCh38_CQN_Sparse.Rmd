---
title: "HBCC - ACC - STG - FEATURECOUNTS - GRCh38 - CQN - Sparse Model"
...
knit2synapse::createAndKnitToFolderEntity(file = "./cmc-rnaseq-de/HBCC_ACC-STG_FEATURECOUNTS_GRCh38_CQN_Sparse.Rmd",
                                          parentId ="syn21435176",
                                          folderName = 'HBCC - ACC - STG - FEATURECOUNTS - GRCh38 - CQN - Sparse Model')
```
...
activityDescription = 'Covariate analysis of GRCh38 feature counts with CQN normalisation (ACC and STG)';

thisFileName <- 'HBCC_ACC-STG_FEATURECOUNTS_GRCh38_CQN_Sparse.Rmd'
...
METADATA_QC_ID = 'syn16816488' 
ALL_USED_IDs = METADATA_QC_ID
metadata = downloadFile_version(METADATA_QC_ID, version = 16) %>%
  dplyr::select(`Individual_ID`, Institution, `Reported_Gender`, Sex, Ethnicity, ageOfDeath, `PMI_(in_hours)`, Dx, 
                Sample_RNA_ID, one_of('Cohort', 'rnaSeq_isolation:RIN',
                                      'rnaSeq_dissection:Brain_Region',
                                      'rnaSeq_isolation:RNA_Isolation_Batch',
                                      'rnaSeq_report:Library_Batch', 
  ...
  dplyr::filter(Cohort %in% c("NIMH-HBCC-4")) %>% 
  dplyr::filter(`rnaSeq_dissection:Brain_Region` %in% c("ACC","STG"))

## Download reprocessed counts (ACC and STG)
counts = 'syn21420322'
...
COUNT$End <- NULL
COUNT$Strand <- NULL
# Fix counts column header - remove "X" in sample name
new_names <- gsub("X6", "6", colnames(COUNT))
colnames(COUNT) <- new_names
...
METADATA = metadata %>%
  ...
  dplyr::rename(Tissue = `rnaSeq_dissection:Brain_Region`,
                PMI = `PMI_(in_hours)`,
  ...
  dplyr::select(SampleID, Individual_ID, Institution, Tissue, Reported_Gender, Sex, Ethnicity, ageOfDeath, PMI, Dx, 
                RIN, ReportExclude, GenotypeExclude, EV.1, EV.2, EV.3, EV.4, EV.5,
                LibraryBatch, FlowcellBatch, IsolationBatch, MappedReads, TotalReads, GenesDetected, AlignmentRate,
                IntragenicRate, IntergenicRate, IntronicRate, ExpProfEfficiency, rRNARate, TranscriptsDetected) %>%
  dplyr::mutate(Dx = forcats::fct_recode(Dx, Other = "MDD", Other = "BP", Control = "Control", SCZ = "SCZ"))

# Limit HBCC ACC and STG Counts
...
ind = METADATA$SampleID [is.na(METADATA$Ethnicity)]
writeLines(paste('Following', length(ind), 'counts are missing ethnicity information'))
writeLines(paste(ind, collapse = ', '))
METADATA <- METADATA  %>% dplyr::filter(!(SampleID %in% ind)) 
...
FactorCovariates <- c('Individual_ID', "Reported_Gender", "IsolationBatch", "Dx.Tissue")
...
```
### Filter genes
Remove genes that have less than 1 cpm counts in at least 50% of samples per Dx.Tissue and Reported Gender. Also remove genes with missing gene length and percentage GC content
...
p <- ggplot(plotdata, aes(x=PC1, y=PC2))
p <- p + geom_point(aes(shape=Dx.Tissue, size=ageOfDeath))
p <- p + theme_bw() %+replace% theme(legend.position="top", plot.title = element_text(hjust=0.5, size = 15)) + ggtitle ("PCA HBCC ACC-STG Cohort") + 
...
p <- p + geom_point(aes(shape=Dx.Tissue, size=ageOfDeath))
p <- p + theme_bw() %+replace% theme(legend.position="top", plot.title = element_text(hjust=0.5, size = 15)) + ggtitle ("MDS HBCC ACC-STG Cohort") + 
  guides(colour = guide_legend(nrow = 2),
         shape = guide_legend(nrow = 2),
         size = guide_legend(nrow = 2))
p
...
COVARIATES.tmp = data.matrix(COVARIATES[,c("Reported_Gender", "Dx.Tissue")])
...
baseModelBIC = fitMixedEffectModel(adjust.covars, 
  ...
  dplyr::rename(baseModel = BIC)
# Iteratively identify covariates in phase I
phase2.covars = c('IsolationBatch')
...
# Eucledian tree based analysis
COVARIATES.tmp = data.matrix(COVARIATES[,c('Dx.Tissue')])
COVARIATES.tmp[is.na(COVARIATES.tmp)] = 0
...
contrast = makeContrasts(contrasts=c("Dx.TissueOther_ACC-Dx.TissueControl_ACC",
                                     ...
                                     "Dx.TissueOther_STG-Dx.TissueControl_STG",
                                     "Dx.TissueSCZ_STG-Dx.TissueControl_STG",
                                     "Dx.TissueSCZ_STG-Dx.TissueOther_STG"),
...
# Code
CODE <- Folder(name = "HBCC - ACC - STG - FEATURECOUNTS - GRCh38 - CQN - Sparse Model", parentId = parentId)
CODE <- synStore(CODE)

# Store covariates
COVARIATES = rownameToFirstColumn(COVARIATES, 'SampleID')
write.table(COVARIATES, file = 'CMC_HBCC_ACC-STG_Covariates.tsv', sep = '\t', row.names=F, quote=F)
COV_OBJ = synapser::File('CMC_HBCC_ACC-STG_Covariates.tsv', name = 'Covariates', parentId = CODE$properties$id)
...
PROCESSED_COUNTS$filteredExprMatrix$counts %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = 'CMC_HBCC_ACC-STG_Counts.tsv', sep = '\t', row.names=F, quote=F)
COUNT_OBJ = File('CMC_HBCC_ACC-STG_Counts.tsv', name = 'Counts (filtered raw)', parentId = CODE$properties$id)
...
CQN.GENE_EXPRESSION$E %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = 'CMC_HBCC_ACC-STG_logCPM.tsv', sep = '\t', row.names=F, quote=F)
LCOUNT_OBJ = File('CMC_HBCC_ACC-STG_logCPM.tsv', name = 'Counts (filtered logCPM)', parentId = CODE$properties$id)
...
CQN.GENE_EXPRESSION$offset %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = 'CMC_HBCC_ACC-STG_offset.tsv', sep = '\t', row.names=F, quote=F)
OFFSET_OBJ = File('CMC_HBCC_ACC-STG_offset.tsv', name = 'Gene length and GC content offset', parentId = CODE$properties$id)
...
DM1$design %>%
  rownameToFirstColumn('SampleID') %>%
  write.table(file = 'CMC_HBCC_ACC-STG_Design.tsv', sep = '\t', row.names=F, quote=F)
DM_OBJ = File('CMC_HBCC_ACC-STG_Design.tsv', name = 'Design Matrix', parentId = CODE$properties$id)
...
RESIDUAL.GENE_EXPRESSION %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = 'CMC_HBCC_ACC-STG_netResidualExpression.tsv', sep = '\t', row.names=F, quote=F)
nEXP_OBJ = File('CMC_HBCC_ACC-STG_netResidualExpression.tsv', 
...
write.table(DE, file = 'CMC_HBCC_ACC-STG_DiffExpression.tsv', sep = '\t', row.names=F, quote=F)
DEXP_OBJ = File('CMC_HBCC_ACC-STG_DiffExpression.tsv', 
...
write.table(enrichment.results, file = 'CMC_HBCC_ACC-STG_EnrichResults.tsv', sep = '\t', row.names=F, quote=F)
ENRICH_OBJ = File('CMC_HBCC_ACC-STG_EnrichResults.tsv', 
