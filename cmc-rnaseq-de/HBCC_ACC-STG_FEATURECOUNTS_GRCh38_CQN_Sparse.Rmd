---
title: "HBCC - ACC - STG - DREAM - FEATURECOUNTS - GRCh38 - CQN - Sparse Model"
...
knit2synapse::createAndKnitToFolderEntity(file = "./cmc-rnaseq-de/HBCC_ACC-STG_FEATURECOUNTS_GRCh38_CQN_Sparse.Rmd",
                                          ...
                                          folderName = 'HBCC - ACC - STG - DREAM - FEATURECOUNTS - GRCh38 - CQN - Sparse Model')
```
...
metadata = downloadFile_version(METADATA_QC_ID, version = 15) %>%
...
p <- ggplot(plotdata, aes(x=PC1, y=PC2))
p <- p + geom_point(aes(shape=Dx.Tissue, size=ageOfDeath))
p <- p + theme_bw() %+replace% theme(legend.position="top") + facet_grid(.~Dx.Tissue, scales = 'free_y')
p
```
...
# Code
CODE <- Folder(name = "HBCC - ACC - STG - DREAM - FEATURECOUNTS - GRCh38 - CQN - Sparse Model", parentId = parentId)
CODE <- synStore(CODE)
