...
knit2synapse::createAndKnitToFolderEntity(file = "./cmc-rnaseq-de/MSSM-Penn-Pitt_ACC_DLPFC_FEATURECOUNTS_GRCh38_CQN_Sparse.Rmd",
                                 parentId ="syn9872607",
                                 folderName = "MSSM-Penn-Pitt - ACC and DLPFC - DREAM - FEATURECOUNTS - GRCh38 - CQN - Sparse Model")
```
...
# Github link
thisRepo <- getRepo(repository = "kelshmo/analysis", ref="branch", refName='compute-tissue-cor')
...
fitMixedEffectModel <- function(varsToFit, groupingVar, lcpm, cnt, covars){
  ...
  modelBIC = foreach::foreach(i = 1:dim(lcpm)[1],
                              ...
                              writeLines(paste("This is the formula:",formula))
...
```{r phase1}
# Fit a base model with the following covariates varsToFit, groupingVar, lcpm, cnt, covars
...
# Residuals after normalisation - residuals.MArrayLM() does not take MArrayLM2 objects
...
``` {r cor.repeat.measures}
repeated <- COVARIATES %>%
  tidyr::separate(Dx.Tissue, c('Dx','Tissue'), sep = '_') %>% 
  dplyr::group_by(Individual_ID, Tissue) %>% 
  dplyr::summarize(Samples = n()) %>% 
  dplyr::filter(Samples == 1) %>% 
  dplyr::group_by(Individual_ID) %>% 
  dplyr::summarize(multiSample_perTissue = n()) %>% 
  dplyr::filter(multiSample_perTissue == 2)

glue::glue("{length(unique(repeated$Individual_ID))} of {length(unique(COVARIATES$Individual_ID))} have repeat measurements")

quant_cor <- function(count_matrix, metadata, repeated_vec){
  correlations <- tribble(~ iid, ~ cor)
  for (i in repeated_vec){
     samps <- metadata$SampleID[metadata$Individual_ID %in% i]
     cors <- cor(count_matrix[,samps[1]], count_matrix[,samps[2]])
     entry <- tribble(~ iid, ~ cor,
                      as.character(i), cors)
     correlations <- bind_rows(correlations,entry)
  }
  correlations
}
...
correlation_tissue <- quant_cor(tmp, METADATA, repeated$Individual_ID)
```
...
variancePartition::plotContrasts(L) 
...
# Code
CODE <- Folder(name = "MSSM-Penn-Pitt - ACC and DLPFC - DREAM - FEATURECOUNTS - GRCh38 - CQN - Sparse Model", parentId = parentId)
CODE <- synStore(CODE)
...
# Store correlation between tissues
write.table(correlation_tissue, file = 'CMC_ACC_DLPFC_correlationTissue.tsv', sep = '\t', row.names = FALSE, quote = FALSE)
COR_OBJ = File('CMC_ACC_DLPC_correlationTissue.tsv',
               name = 'Correlation between Tissues, Same Individual',
               parentId = CODE$properties$id)
COR_OBJ = synStore(COR_OBJ, used = ALL_USED_IDs, activityName = activityName,
                   executed = thisFile, activityDescription = acactivityDescription)
synSetAnnotations(COR_OBJ, annotations = all.annotations)
```
```
|  *Results*                                  |  *SynapseID*                     |
|  -----------------------------------------  |   ---------                      |
