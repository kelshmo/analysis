...
knit2synapse::createAndKnitToFolderEntity(file = "./HBCC_DLPFC_FEATURECOUNTS_GRCh38_CQN_Sparse.Rmd",
...
metadata = downloadFile_version(METADATA_QC_DLPFC_ID, version = 13) %>%
...
## Mapping file for misnamed HBCC samples 
maps <- downloadFile("syn17115622") %>% 
  dplyr::filter()
...
# Map to HBCC Sample Ids
METADATA <- dplyr::select(maps, ID, RNA_ID) %>% 
  dplyr::right_join(METADATA, by = c("RNA_ID" = "SampleID")) %>% 
  dplyr::mutate(SampleID = dplyr::coalesce(ID, RNA_ID))
