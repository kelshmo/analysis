---
title: "HBCC - ACC-Nirmala - FEATURECOUNTS - GRCh38 - CQN - Sparse Model"
...
knit2synapse::createAndKnitToFolderEntity(file = "./cmc-rnaseq-de/HBCC_ACC-Nirmala_FEATURECOUNTS_GRCh38_CQN_Sparse.Rmd",
                                          parentId ="syn21435176",
                                          folderName = 'HBCC - ACC-Nirmala - DREAM - FEATURECOUNTS - GRCh38 - CQN - Sparse Model')
```
...
activityDescription = 'Covariate analysis of GRCh38 feature counts with CQN normalisation (ACC-Nirmala)';

thisFileName <- 'HBCC_ACC-Nirmala_FEATURECOUNTS_GRCh38_CQN_Sparse.Rmd'
...
metadata = downloadFile_version(METADATA_QC_ACC_ID, version = 16) %>%
  ...
  dplyr::filter(`rnaSeq_dissection:Brain_Region` %in% c("ACC")) %>% 
  dplyr::filter(Cohort %in% c("NIMH-HBCC-5"))
...
GENOTYPE = downloadFile(GENOTYPE_ID) %>% 
  ...
  dplyr::filter(is.na(`SNP_report:Exclude?`)) #Remove excluded samples here because there are duplicate individuals
```
...
METADATA = metadata %>%
  ...
  dplyr::rename(Region = `rnaSeq_dissection:Brain_Region`,
                ...
                IsolationBatch = `rnaSeq_isolation:RNA_Isolation_Batch`,
  ...
  dplyr::select(SampleID, Individual_ID, Institution, Reported_Gender,Sex, Ethnicity, ageOfDeath, PMI, Dx, 
                ...
                LibraryBatch, IsolationBatch, RibozeroBatch, MappedReads, TotalReads, GenesDetected, AlignmentRate,
                IntragenicRate, IntergenicRate, IntronicRate, ExpProfEfficiency, rRNARate, TranscriptsDetected) %>%
  dplyr::mutate(Dx = forcats::fct_recode(Dx, Other = "BP", Control = "Control", SCZ = "SCZ"))

# Limit HBCC ACC Counts
...
ind = METADATA$SampleID [is.na(METADATA$RIN)]
writeLines(paste('Following', length(ind), 'counts are missing RIN'))
writeLines(paste(ind, collapse = ', '))
METADATA <- METADATA  %>% dplyr::filter(!(SampleID %in% ind))
...
FactorCovariates <- c('Individual_ID', "Reported_Gender", "LibraryBatch", "Dx", "IsolationBatch")
...
p <- ggplot(plotdata, aes(x=PC1, y=PC2))
p <- p + geom_point(aes(shape=Dx, size=ageOfDeath))
p <- p + theme_bw() %+replace% theme(legend.position="top", plot.title = element_text(hjust=0.5, size = 15)) + ggtitle ("PCA HBCC Cohort") + 
...
p <- p + geom_point(aes(shape=Dx, size=ageOfDeath))
p <- p + theme_bw() %+replace% theme(legend.position="top", plot.title = element_text(hjust=0.5, size = 15)) + ggtitle ("HBCC Cohort") + 
  guides(colour = guide_legend(nrow = 2),
         shape = guide_legend(nrow = 2),
         size = guide_legend(nrow = 2))
p
...
selectCovarsToAdjust <- function(covarsToSelect, adjustCovars, covars, lcpm, cnt, baseModelBIC){
  ...
  while(length(covarsToSelect) != 0){
    allModelBIC = foreach(i = 1:length(covarsToSelect),
                          .combine = c,
                          .packages = pkgs,
                          .export = c('fitFixedEffectModel')) %dopar% {
...
baseModelBIC = fitFixedEffectModel(adjust.covars, 
  ...
  dplyr::rename(baseModel = BIC)
# Iteratively identify covariates in phase I
phase2.covars = c('LibraryBatch', 'IsolationBatch')
...
L1 = variancePartition::getContrast(exprObj = VOOM.GENE_EXPRESSION, 
                                   formula = formula, 
                                   data = COVARIATES, 
                                   coefficient = c("Dx.TissueOther", "Dx.TissueControl"))
L2 = variancePartition::getContrast(exprObj = VOOM.GENE_EXPRESSION, 
                                   formula = formula, 
                                   data = COVARIATES, 
                                   coefficient = c("Dx.TissueSCZ", "Dx.TissueControl"))
L3 = variancePartition::getContrast(exprObj = VOOM.GENE_EXPRESSION, 
                                   formula = formula, 
                                   data = COVARIATES, 
                                   coefficient = c("Dx.TissueSCZ", "Dx.TissueOther"))
L = cbind(L1, L2, L3)
...
# Code
CODE <- Folder(name = "HBCC - ACC-Nirmala - DREAM - FEATURECOUNTS - GRCh38 - CQN - Sparse Model", parentId = parentId)
CODE <- synStore(CODE)

# Store covariates
COVARIATES = rownameToFirstColumn(COVARIATES, 'SampleID')
write.table(COVARIATES, file = 'CMC_HBCC_ACC-Nirmala_Covariates.tsv', sep = '\t', row.names=F, quote=F)
COV_OBJ = synapser::File('CMC_HBCC_ACC-Nirmala_Covariates.tsv', name = 'Covariates', parentId = CODE$properties$id)
...
PROCESSED_COUNTS$filteredExprMatrix$counts %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = 'CMC_HBCC_ACC-Nirmala_Counts.tsv', sep = '\t', row.names=F, quote=F)
COUNT_OBJ = File('CMC_HBCC_ACC-Nirmala_Counts.tsv', name = 'Counts (filtered raw)', parentId = CODE$properties$id)
...
CQN.GENE_EXPRESSION$E %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = 'CMC_HBCC_ACC-Nirmala_logCPM.tsv', sep = '\t', row.names=F, quote=F)
LCOUNT_OBJ = File('CMC_HBCC_ACC-Nirmala_logCPM.tsv', name = 'Counts (filtered logCPM)', parentId = CODE$properties$id)
...
CQN.GENE_EXPRESSION$offset %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = 'CMC_HBCC_ACC-Nirmala_offset.tsv', sep = '\t', row.names=F, quote=F)
OFFSET_OBJ = File('CMC_HBCC_ACC-Nirmala_offset.tsv', name = 'Gene length and GC content offset', parentId = CODE$properties$id)
...
DM1$design %>%
  rownameToFirstColumn('SampleID') %>%
  write.table(file = 'CMC_HBCC_ACC-Nirmala_Design.tsv', sep = '\t', row.names=F, quote=F)
DM_OBJ = File('CMC_HBCC_ACC-Nirmala_Design.tsv', name = 'Design Matrix', parentId = CODE$properties$id)
...
RESIDUAL.GENE_EXPRESSION %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = 'CMC_HBCC_ACC-Nirmala_netResidualExpression.tsv', sep = '\t', row.names=F, quote=F)
nEXP_OBJ = File('CMC_HBCC_ACC-Nirmala_netResidualExpression.tsv', 
...
write.table(DE, file = 'CMC_HBCC_ACC-Nirmala_DiffExpression.tsv', sep = '\t', row.names=F, quote=F)
DEXP_OBJ = File('CMC_HBCC_ACC-Nirmala_DiffExpression.tsv', 
...
write.table(enrichment.results, file = 'CMC_HBCC_ACC-Nirmala_EnrichResults.tsv', sep = '\t', row.names=F, quote=F)
ENRICH_OBJ = File('CMC_HBCC_ACC-Nirmala_EnrichResults.tsv', 
                  name = 'Enrichment Analysis Results (Dx)', 
                  parentId = CODE$properties$id)
