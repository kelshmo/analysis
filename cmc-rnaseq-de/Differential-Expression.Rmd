...
adjust.covars <- c("Dx.Tissue", "RIN", "IntragenicRate")
```
...
df_stg = rownameToFirstColumn(COVARIATES, "SampleID") %>% 
  filter( Individual_ID %in% names(hasReps[hasReps==2])) %>%
  filter(grepl("STG", Dx.Tissue)) %>% 
...
df_merge = merge(df_stg, df_acc, by="Individual_ID")
...
hist(corRes, xlab="Correlation", main="Correlation between ACC and STG sample from the sample individual for each gene")
...
RESIDUAL.GENE_EXPRESSION %>%
  ...
  write.table(file = 'CMC_ACC_STG_Dx.Tissue_netResidualExpression.tsv', sep = '\t', row.names=F, quote=F)
nEXP_OBJ = File('CMC_ACC_STG_Dx.Tissue_netResidualExpression.tsv', 
...
write.table(DE, file = 'CMC_ACC_STG_Dx.Tissue_DiffExpression.tsv', sep = '\t', row.names=F, quote=F)
DEXP_OBJ = File('CMC_ACC_STG_Dx.Tissue_DiffExpression.tsv', 
...
saveRDS(ADJUSTED.FIT, file = 'CMC_ACC_STG_Dx.Tissue_dream-output.rds')
ADJ_OBJ = File('CMC_ACC_STG_Dx.Tissue_dream-output.rds',
