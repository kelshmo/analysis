...
metadata <- metadata %>%
  ...
  dplyr::rename(age_death = AgeAtDeath) %>%
  ...
  dplyr::mutate(tissue_diagnosis = paste(tissue, diagnosis, sep = "_")) %>%
  dplyr::rename(source = Source) %>%
  dplyr::mutate(source_tissue_diagnosis = paste(source, tissue_diagnosis, sep = "_")) %>%
  dplyr::rename(sex = Gender) %>%
  dplyr::mutate(APOE4 = NA)
...
# Predict missing PMI
PMI <- metadata[,c("PMI", "ID", "Donor_ID")]
